...
case class RawMoveMessage(stringToId: Map[String, String])
...
class Hand extends CometActor with CometListener {
  ...
  override def lowPriority: PartialFunction[Any, Unit] = {
    ...
    case rmm: RawMoveMessage => {  Game ! MoveMessage(List("card1", "card2", "card3", "card4", "card5") map rmm.stringToId map UUID.fromString map cards)}
  }
